${request.setHeader('TradeId', xml["/MM/TradeId/text()"])}
...
${xml["/MM/Env/ENV/Desk"]},
${xml["/MM/Env/ENV/Cust"]},
<#if xml["/MM/Env/ENV/Folder"][0]??>${xml["/MM/Env/ENV/Folder"]}</#if>,
${xml["/MM/Env/ENV/TradeDate"]},
<#if xml["/MM/Env/ENV/Trader"][0]??>${xml["/MM/Env/ENV/Trader"]}</#if>,
<#if xml["/MM/Env/ENV/StructureId"][0]??>${xml["/MM/Env/ENV/StructureId"]}</#if>,
<#if xml["/MM/Env/ENV/Comment1"][0]??>${xml["/MM/Env/ENV/Comment1"]}</#if>,
<#--  MM specific -->
<#list xml["/MM/Assets/ASSET"] as asset>
...
<#if asset.ROLL_RolloverStatus[0]??>${asset.ROLL_RolloverStatus}</#if>,
<#if asset.INT_ACC_DP[0]??>${asset.INT_ACC_DP}</#if>
<#-- cashflows -->
<#--list asset.SCHEDULE.Row as flow>${flow.Date}|${flow.Notional}|${flow.Start}|${flow.End}||||<#if flow.Fixing[0]??>${flow.Fixing}</#if>|${flow.Spread}|||<#sep>;</#sep></#list-->
<#sep>,</#sep>
</#list>
</@compress>